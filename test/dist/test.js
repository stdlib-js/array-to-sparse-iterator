...
var sparsearray2iterator = require( './../../dist' );
...
tape( 'the function throws an error if provided a first argument which is not an array-like object (callback)', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			sparsearray2iterator( value, noop );
		};
	}
});
...
tape( 'the function throws an error if provided a second argument which is not a function', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			sparsearray2iterator( [ 1, 2, 3, 4 ], value );
		};
	}
});
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	values = [ 1, , , 4 ]; // eslint-disable-line no-sparse-arrays
	expected = [
		{
			'value': 1,
			'done': false
	...
	it = sparsearray2iterator( values );
...
tape( 'the function returns an iterator protocol-compliant object (empty array)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;

	values = new Array( 1e5 );
	expected = [
		{
			'done': true
		}
...
tape( 'the function returns an iterator protocol-compliant object (empty end)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
...
tape( 'the function returns an iterator protocol-compliant object (empty start)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
...
tape( 'the function returns an iterator protocol-compliant object (empty start+end)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	// Leave the first and last elements empty:
...
tape( 'the function returns an iterator protocol-compliant object which supports invoking a provided function for each iterated value', function test( t ) {
	...
	it = sparsearray2iterator( values, scale );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object', function test( t ) {
	...
	values = [ 1, , 3, 4 ]; // eslint-disable-line no-sparse-arrays
	expected = [
		{
			'value': 1,
			'done': false
...
tape( 'the function returns an iterator protocol-compliant object (callback)', function test( t ) {
	...
	values = [ 1, 2, , 4 ]; // eslint-disable-line no-sparse-arrays
	expected = [
		{
			'value': 1,
			'done': false
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable', function test( t ) {
	var sparsearray2iterator;
	var values;
	var it1;
	var it2;
	var i;

	sparsearray2iterator = proxyquire( './../dist/main.js', {
		'@stdlib/symbol-iterator': '__ITERATOR_SYMBOL__'
	});

	values = [ 1, 2, 3, 4 ];

	it1 = sparsearray2iterator( values );
...
tape( 'if an environment supports `Symbol.iterator`, the returned iterator is iterable (callback)', function test( t ) {
	...
	it1 = sparsearray2iterator( values, scale );
	t.equal( typeof it1[ '__ITERATOR_SYMBOL__' ], 'function', 'has method' );
	t.equal( it1[ '__ITERATOR_SYMBOL__' ].length, 0, 'has zero arity' );
